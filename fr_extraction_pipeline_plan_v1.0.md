...
A production pipeline that processes the entire corpus of Federal Register documents (1936–present, 1M+ PDFs) through AWS Bedrock to produce a structured relational dataset suitable for estimating the economic impact of each regulation. The pipeline has two processing tiers: a high-throughput batch extraction path for straightforward documents, and an agentic enrichment path for documents that require multi-step reasoning, cross-document lookups, or derived calculations.
...
**Implication for Parquet partitioning:** Partition by `year/document_type/` rather than `year/agency/document_type/` — the most common filter is time period + document type, and agency filtering happens in-memory after partition pruning. Too many partition levels create too many small files.
...
│                  │  │    extract_table, compute_estimate,            │
│                  │  │    lookup_naics, lookup_cfr,                   │
...
The scraper (`fr_scraper_lambda.py`) runs as an AWS Lambda function triggered daily by EventBridge. It uses GovInfo sitemaps to discover Federal Register packages, downloads individual granule PDFs and full-issue PDFs, and uploads them to S3.
...
| Bulk inference | Bedrock Batch API, not real-time | 50% cost reduction, throughput management handled by AWS |
| Text extraction | PyMuPDF on EC2/Batch, not Lambda | PDFs are large, extraction is CPU-bound, Lambda timeout/memory limits are binding |
...
3. **AWS Batch job definition** (for scale-up) — Docker container with PyMuPDF, array job configuration
   - Output to `s3://<BUCKET>/extracted_text/YYYY/MM/DD/<document_id>.json`
   - Not deployed until test sample passes quality review

4. **Integration with daily scraper** (for scale-up) — New PDFs trigger extraction automatically via S3 event notification
...
- [ ] Batch job definition and Docker setup (for scale-up)

---

### Phase 2: Tier 1 Batch Extraction

**Goal:** Process extracted text through Bedrock to produce structured JSON per the extraction schema, using batch inference for cost efficiency.
...
1. **Triage prompt and pipeline (Haiku)**
   ...
   - Implementation: Bedrock Batch API, JSONL input prepared by Lambda, results to S3
...
2. **Full extraction prompt and pipeline (Sonnet)**
   ...
   - Implementation: Bedrock Batch API for filtered documents
...
5. **DynamoDB tracking table** — `fr-extraction-tracking`
   ...
   - Attributes: `extraction_status` (pending | triage_complete | extracted | enriched | failed), `schema_version`, `extraction_tier`, `last_processed`, `confidence_summary`, `s3_output_key`
...
- [ ] Step Functions state machine definition (ASL JSON or CDK)
...
**Goal:** Flatten nested extraction JSON into relational tables accessible from R via Athena.
...
1. **ETL Lambda** (`fr-parquet-writer`) — Triggered by new extraction results in S3
   - Reads nested JSON
   - Writes partitioned Parquet files: one file per partition (year/agency/document_type)
   - Tables: `documents`, `economic_estimates`, `affected_industries`, `regulatory_instruments`, `cross_references`, `extraction_metadata`
   
2. **Glue Data Catalog** (`fr_structured` database) — Table definitions for each output table
   
3. **Athena views** — Pre-built analytical views:
...
4. **R integration** — `noctua` or `RAthena` package configuration for querying from R scripts
...
**Steps:**
1. Run Phase 1 (text extraction) on full corpus via AWS Batch array jobs
2. Run Phase 2 (triage + extraction) via Bedrock Batch API — submit in batches of ~10K documents
3. Monitor DynamoDB tracking table for failures, throttling, edge cases
4. Run Phase 3 (agent enrichment) on accumulated low-confidence queue
5. Run Phase 4 (ETL) on all extraction outputs
6. Spot-check a random sample of full-corpus outputs against test-sample quality benchmarks
...
**After backfill:** The pipeline transitions to steady-state: new documents from the daily scraper flow through automatically, and schema updates trigger targeted reprocessing via version comparison.
...
| S3 prefix | `output_tables/` | 4 | Partitioned Parquet files |
...
| Step Functions | `fr-extraction-pipeline` | 2 | Main orchestration state machine |
...
| Batch job def | `fr-pdf-extraction` | 1 | Parallel text extraction (scale-up only) |
...
### 7.3 Ongoing monthly costs

| Component | Estimated monthly |
|-----------|-------------------|
| Daily new document processing (~200 docs/day) | ~$5 |
| S3 storage (all prefixes, ~1 TB total) | ~$23 |
| DynamoDB (on-demand, ~1M items, light traffic) | ~$5 |
| Lambda compute | ~$2 |
...
Development proceeds on the 500-doc test sample through Phases 0–5. Full-corpus processing (Phase 6) is gated on Phase 5 go/no-go. See Section 1.7 for the two-track workflow.
...
| 2 | Step Functions state machine | NOT STARTED | — | |
...
| 4 | Glue catalog + Athena | NOT STARTED | — | |
...
│   ├── parquet_writer.py          # ETL: JSON → Parquet
